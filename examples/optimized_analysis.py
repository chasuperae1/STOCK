...
class GoldAnalyzer:
    def __init__(self):
        ...
        self.gold_keywords = [
            'gold', 'XAU', 'gold price', 'gold market', 'gold futures',
            'gold investment', 'gold ETF', 'gold mining', 'gold bullion',
            'central bank gold', 'gold reserve', 'gold demand', 'gold supply',
            'gold trading', 'gold rally', 'gold surge', 'gold drop', 'gold decline'
        ]
    ...
    def fetch_news(self):
        ...
        search_queries = ["gold price", "gold market", "gold futures", "XAU USD", "gold investment"]
    ...
    def _calculate_relevance(self, title, description):
        ...
        strong_terms = ['gold price', 'xau/usd', 'gold futures', 'gold spot']
        medium_terms = ['gold', 'xau', 'precious metal', 'bullion']
    ...
    def analyze_sentiment(self):
        ...
        positive_keywords = ['surge', 'rise', 'gain', 'bullish', 'high', 'rally', 'boost', 'record', 'jump', 'up', 'increase', 'strong', 'positive', 'upbeat', 'optimistic', 'soar', 'climb', 'advance', 'recovery', 'higher', 'strengthen']
        negative_keywords = ['fall', 'drop', 'decline', 'plunge', 'bearish', 'low', 'crash', 'slump', 'tumble', 'down', 'decrease', 'weak', 'negative', 'worried', 'pessimistic', 'plummet', 'slip', 'loss', 'dip', 'lower', 'weaken']
...
def main():
    ...
    if news:
        ...
        for i, article in enumerate(news[:5], 1):
            ...
            print(f"\n  {i}. [{source}] {title[:80]}...")
            print(f"      相关性: {'★' * relevance}{'☆' * (5 - relevance)}")
    ...
    print(f"📊 波动率风险: {game['volatility_risk']}")
